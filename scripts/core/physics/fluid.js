...
export default class Fluid {
	...
	addVelocity = (
		...
		// Support a vector in place of separate X and Y components.
		if ( typeof vx === 'object' ) {
			radius = vy ?? radius; // Shift arguments if vy is actually radius
			vy = Number( vx.y );
			vx = Number( vx.x );
		}
	...
	addSource = (
		...
		const x = Number( point?.x ?? point );
		const y = Number( point?.y ?? 0 );

		this.addDensity(x, y, density, radius);
		this.addVelocity(x, y, velocity, null, radius);
