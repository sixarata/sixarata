...
import Fluid from '../scripts/core/physics/fluid.js';
...
import Smoke from '../scripts/core/physics/smoke.js';
...
test( 'Fluid accepts scalar and vector-like sources', () => {
	const fluid = new Fluid( 8, 8, 1, { cellSize: 1 } );

	fluid.addDensity( 3, 3, 2, 1 );
	fluid.addVelocity( 3, 3, new Velocity( 2, -1 ), 1 );

	assert.equal( fluid.getDensityAt( 3, 3 ), 2 );
	assert.deepEqual( fluid.getVelocityAt( 3, 3 ), { x: 2, y: -1 } );
} );

test( 'Smoke retains finite state after a simulation step', () => {
	const smoke = new Smoke( 8, 8, 1, { cellSize: 1 } );

	smoke.addSmoke( 3, 3, 1, 1, { r: 1, g: 0.5, b: 0 }, { x: 1, y: -1 }, 1 );
	smoke.step( 0.016 );

	assert.ok( smoke.density.some( value => value > 0 ) );
	assert.ok( smoke.density.every( Number.isFinite ) );
	closeTo( smoke.dt, 0.016 );
} );
